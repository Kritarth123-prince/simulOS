...
int FileSystemObject::remove()
{
    ...
    if (!hasPermission('w'))
    {
        FileSystemError("Permission denied: '" + name + "' is not writable (chmod to add 'w').");
        return -1;
    }
...
int FileSystemObject::displayInfo()
{
    ...
    std::cout << "Permissions: " << getPermissions() << std::endl;
    std::cout << "Size: " << getSize() << ((isDir()) ? " item(s)" : " byte(s)") << std::endl;
...
/*
 *  Validates a permission string: exactly 3 characters,
...
bool permissionStringValidator(const std::string &perms)
{
    ...
    if (perms.at(0) != 'r' && perms.at(0) != '-') return false;
    if (perms.at(1) != 'w' && perms.at(1) != '-') return false;
    if (perms.at(2) != 'x' && perms.at(2) != '-') return false;
    return true;
}

std::string FileSystemObject::getPermissions() const
{
    return permissions;
}

int FileSystemObject::setPermissions(std::string new_perms)
{
    if (!permissionStringValidator(new_perms))
    {
        FileSystemError("Invalid permission string. Expected format like 'rwx', 'r--', 'rw-'.");
        return -1;
    }
...
bool FileSystemObject::hasPermission(char perm) const
{
    if (perm == 'r') return permissions.at(0) == 'r';
    if (perm == 'w') return permissions.at(1) == 'w';
    if (perm == 'x') return permissions.at(2) == 'x';
    return false;
}
...
int DirectoryObject::displayContent() const
{
    if (!hasPermission('r'))
    {
        FileSystemError("Permission denied: '" + getName() + "' is not readable (chmod to add 'r').");
        return -1;
    }
...
/*
 *  Detailed, `ls -l`-style listing: type + permissions, size, last-updated
 *  timestamp, and name for every child object.
 */
int DirectoryObject::displayDetailedContent() const
{
    if (!hasPermission('r'))
    {
        FileSystemError("Permission denied: '" + getName() + "' is not readable (chmod to add 'r').");
    ...
    std::cout << "total " << childObjects.size() << std::endl;
    for (FileSystemObject *child : childObjects)
    {
        std::cout << (child->isDir() ? "d" : "-") << child->getPermissions() << "  ";
        std::cout << child->getSize();
        std::cout << "\t" << child->getUpdatedTime() << "\t";
        std::cout << child->getName() << (child->isDir() ? "/" : "") << std::endl;
    }
    return 0;
}
...
{
    return static_cast<int>(childObjects.size());
}
...
int FileObject::rewrite(std::string new_content)
{
    if (!hasPermission('w'))
    {
        FileSystemError("Permission denied: '" + getName() + "' is not writable (chmod to add 'w').");
        return -1;
    }
    content = new_content;
    changeUpdatedTime();
